    ...
    <script type="text/javascript">
        ...
        function init(x, y) {
            $(document).ready(function() {
                ...
                try
                {
                    $bdd = new PDO('mysql:host=localhost;dbname=bestWestern;charset=utf8', 'root', 'root');
                    $i = 1;
                } catch (Exception $e) {
                    die('Erreur : ' . $e->getMessage());
                }
                $reponse = $bdd->query('SELECT * FROM Hostels');
                ...
                while ($donnees = $reponse->fetch())
                {
                    ...
                    $(".prix_show").hide();
                    cord_hotel = new google.maps.LatLng(<?php echo $donnees['coordx']; ?>, <?php echo $donnees['coordy']; ?>);
                    markerz[<?php echo $i; ?>] = new google.maps.Marker({
                        position: cord_hotel,
                        map: maCarte,
                        icon: image,
                        title: '<?php echo $donnees['category'] . " " . $donnees['name']?>',
                        animation: null
                    });
                    markersArray.push(markerz[<?php echo $i; ?>]);
                    $("#liste_hotels").append('<li id="opener<?php echo $i; ?>" onmouseleave="outhover(this)" onmouseenter="hoverLi(this);" onclick="gotoMapMarker(<?php echo $i; ?>); clickLi(this);"><div class="visuelListeHotels"><a style="cursor: pointer;" class="nounderline"><img src="http://www.bestwestern.fr/public/images/pictos_miles/hotel_gamme/bw16_mini.png "/></a></div><div class="contentListeHotels"><a href="javascript:void(0)" class="nounderline"><?php echo $donnees['category'] . " " . $donnees['name'];?>, Paris &nbsp;' + e + '<span> <?php echo $donnees['address'] . ", " . $donnees['zipCode'] . ", " . $donnees['city'];?>, France</span></a></div><div class="clr"></div></li>');
                    buildInfoWindow(maCarte, cord_hotel, markerz[<?php echo $i; ?>], '<?php echo $donnees['coord']; ?>', '<?php echo $donnees['category']; ?>', '<?php echo $donnees['category'] . " " . $donnees['name']?>', '<?php echo $donnees['city']; ?>', 'France', e, '<?php echo $donnees['address']; ?>', '<?php echo $donnees['zipCode']; ?>', 'http://www.bestwestern.fr/infos_hotel.jsp?HotelCode=<?php echo $donnees['coord']; ?>&amp;primarylangid=fr-FR&amp;RequestedCurrencyCode=EUR&amp;ChainCode=BW&amp;roomStay=1', image_facade_hotel, '0', '', '', '4', '526');
