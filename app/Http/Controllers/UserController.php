...
class UserController extends Controller {
	...
	public function selfshow() {
		...
		$user = $this->userRepository->findById($id);
		$convs = TBMsg::getUserConversations($id);
		...
		if (count($convs) > 0) {
			foreach ($convs as $conv) {
				$messages += count($conv->getAllMessages());
				$last = new stdClass();
				...
				if (!empty($participants)) {
					$participant = $this->userRepository->whereIn('id', $participants)->where('id', '!=', $id)->first();
					$last->participant = $participant;
					$last->countMessages = count($conv->getAllMessages());
					$last->unread = TBMsg::getNumOfUnreadMsgs($id);
					$last->lastMessage = $conv->getLastMessage();
					$last->id = $conv->getId();
					array_push($history, $last);
				}
			}
			$lastConv = array_pop($history);
			if (count($history) > 2) {
	...
	public function selfedit() {
		$user = $this->userRepository->findById(Sentinel::getUser()->id);
		return view('Centaur::user.edit', [
			'user' => $user,
		]);
	}
